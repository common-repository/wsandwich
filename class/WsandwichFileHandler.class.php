...
class WsandwichFileHandler {
	...
	function setPath($path){
		...
		if($path == '/' || !$path)  die('WsandwichFileHandler->setPath() :: 디렉토리 이름이 없습니다.');
	...
	function checkPath($path, $mk = true){
		$path = WSANDWICH_ROOT_DIR . $this->addFirstSlash($path);
		if(!file_exists($path)){
	...
	function mkPath($path, $permission=0777){
		$path = str_replace(WSANDWICH_ROOT_DIR . '/', '', $path);
		$growing_path = WSANDWICH_ROOT_DIR;
		$path = explode('/', $path);
	...
	function upload($name, $extension=array(), $extension_type=1, $limit_file_size=10485760){
		...
		*/
		...
		if(!$this->path) die('WsandwichFileHandler->upload() :: 디렉토리 경로가 없습니다.');
	...
	private function singleUpload($file){
		if($file["size"] >= 0 && $file["name"]){
			...
			if(!@move_uploaded_file($file["tmp_name"], WSANDWICH_ROOT_DIR . $this->path. "/" .$this->file_unique_name)){
	...
	private function multipleUpload($file){
		for($i=0, $cnt=count($file["name"]); $i<$cnt; $i++){
			...
			if($file["size"][$i] >= 0 && $file["name"][$i]){
				...
				if(!@move_uploaded_file($file["tmp_name"][$i], WSANDWICH_ROOT_DIR . $this->path. "/" .$this->file_unique_name)){
	...
	function download($file){
		...
		if(!$this->path) die('WsandwichFileHandler->download() :: 디렉토리 경로가 없습니다.');
		$fullPath = WSANDWICH_ROOT_DIR . $this->path . '/' . $file;
	...
	function remove($file){
		if(!$this->path) die('WsandwichFileHandler->delete() :: 디렉토리 경로가 없습니다.');
		return @unlink(WSANDWICH_ROOT_DIR . $this->path . $this->addFirstSlash($file));
	}
